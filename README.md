...
graph TB
    ...
    B --> B2[Batch<br/>批量转换]
    B --> B3[Registry<br/>注册中心]

    C --> C1[Transformer<br/>字段转换器]
    C --> C2[Validator<br/>参数校验]
    C --> C3[EnumMapper<br/>枚举映射]
    C --> C4[Desensitize<br/>脱敏转换]
    C --> C5[SafeConverter<br/>安全转换]

    D --> D1[Errors<br/>类型化错误]
    D --> D2[Options<br/>选项模式]
    D --> D3[Helpers<br/>反射工具]
    D --> D4[Time<br/>时间转换]

    E --> E1[go-toolbox/syncx<br/>并发安全Map]
    E --> E2[go-toolbox/errorx<br/>类型化错误]
    E --> E3[go-toolbox/safe<br/>安全访问]
    E --> E4[go-toolbox/desensitize<br/>数据脱敏]
...
```mermaid
flowchart LR
    PB[Protocol Buffer] --> |ConvertPBToModel| M[Model Object]
    M --> |ConvertModelToPB| PB
